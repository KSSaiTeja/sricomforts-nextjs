...
import { keepBrandTogether, lockBrandNameInElement } from "@/lib/brand";
  ...
  useGSAP(
    () => {
      ...
      sections.forEach((section, index) => {
        ...
        if (title && !animatedTitles.has(title) && !title.classList.contains("title-sequence")) {
          animatedTitles.add(title);
          lockBrandNameInElement(title);
          const titleInView =
            sectionInView || title.getBoundingClientRect().top < viewportHeight * 0.92;

          // Title chars only — never hide the rest of the section (that was the white flash).
          if (lightMotion) {
            gsap.fromTo(
              title,
              { autoAlpha: 0, y: 22 },
        ...
        // Homepage blocks (catalog, logos, features) keep their own motion.
        if (!isHome && items.length) {
          gsap.fromTo(
            items,
            { y: lightMotion ? 16 : 24 },
            {
